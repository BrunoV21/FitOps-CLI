...
class ZoneResult:
    ...
    zones: list[Zone]
    lt1_bpm: Optional[int]
    lt2_bpm: Optional[int]
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "heart_rate_zones": [
                {"zone": z.zone, "name": z.name, "min_bpm": z.min_bpm, "max_bpm": z.max_bpm, "description": z.description}
                for z in self.zones
            ],
            "thresholds": {
                "lt1_bpm": self.lt1_bpm,
                "lt2_bpm": self.lt2_bpm,
                "lt1_label": f"Top of Zone 2 (92% LTHR = {self.lt1_bpm} bpm)" if self.lt1_bpm else None,
                "lt2_label": f"LTHR (100% LTHR = {self.lt2_bpm} bpm)" if self.lt2_bpm else None,
            },
        }
...
def compute_max_hr_zones(max_hr: int) -> ZoneResult:
    zones = [
        Zone(1, "Recovery",  int(max_hr * 0.50), int(max_hr * 0.60), "50–60% max HR — recovery"),
        Zone(2, "Aerobic",   int(max_hr * 0.60), int(max_hr * 0.70), "60–70% max HR — aerobic base"),
        Zone(3, "Tempo",     int(max_hr * 0.70), int(max_hr * 0.80), "70–80% max HR — aerobic threshold"),
        Zone(4, "Threshold", int(max_hr * 0.80), int(max_hr * 0.90), "80–90% max HR — lactate threshold"),
        Zone(5, "VO2max",    int(max_hr * 0.90), 999,                "90–100% max HR — high intensity"),
    ]
    return ZoneResult(method="max-hr", lthr_bpm=None, max_hr_bpm=max_hr, resting_hr_bpm=None,
                      zones=zones, lt1_bpm=int(max_hr * 0.70), lt2_bpm=int(max_hr * 0.85))
...
def compute_hrr_zones(max_hr: int, resting_hr: int) -> ZoneResult:
    ...
    return ZoneResult(method="hrr", lthr_bpm=None, max_hr_bpm=max_hr, resting_hr_bpm=resting_hr,
                      zones=zones, lt1_bpm=t(0.70), lt2_bpm=t(0.85))
