...
def _fmt_pace(s: int | None) -> str | None:
    if s is None:
        return None
    s = int(s)
...
def compute_pace_zones(threshold_pace_s: int, source: str = "manual") -> PaceZoneResult:
    ...
    for zone_num, name, faster_pct, slower_pct in PACE_ZONE_DEFS:
        min_s = (
            int(round(threshold_pace_s * faster_pct))
            if faster_pct is not None
            else None
        )
        max_s = (
            int(round(threshold_pace_s * slower_pct))
            if slower_pct is not None
            else None
        )
        zones.append(
            {
...
def get_pace_zones() -> PaceZoneResult | None:
