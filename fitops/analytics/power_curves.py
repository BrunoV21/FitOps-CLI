...
CP_ZONE_NAMES = {
    1: "Active Recovery", 2: "Endurance", 3: "Tempo",
    4: "Lactate Threshold", 5: "VO2max", 6: "Neuromuscular",
}


def _max_mean(data: list[float], window: int) -> Optional[float]:
    if len(data) < window:
        return None
...
def _fit_cp(durations: list[int], powers: list[float]) -> tuple[Optional[float], Optional[float], Optional[float]]:
    try:
    ...
    try:
        ...
        ss_res = sum((y - yp) ** 2 for y, yp in zip(powers, y_pred))
...
def _cp_zones(cp: float) -> list[dict]:
    limits = [(1, 0.0, 0.55), (2, 0.56, 0.75), (3, 0.76, 0.90),
              (4, 0.91, 1.05), (5, 1.06, 1.20), (6, 1.50, None)]
    zones = []
...
async def compute_power_curve(
    ...
    async with get_async_session() as session:
        ...
        ath = (await session.execute(select(Athlete).where(Athlete.strava_id == athlete_id))).scalar_one_or_none()
