...
def _register_subapps() -> None:
    ...
    app.add_typer(auth_app, name="auth", help=f"Manage Strava authentication.\n\nDocs: {CLI_DOCS['auth']}")
    app.add_typer(sync_app, name="sync", help=f"Sync activities from Strava.\n\nDocs: {CLI_DOCS['sync']}")
    app.add_typer(activities_app, name="activities", help=f"View synced activities.\n\nDocs: {CLI_DOCS['activities']}")
    app.add_typer(athlete_app, name="athlete", help=f"View athlete profile and stats.\n\nDocs: {CLI_DOCS['athlete']}")
    app.add_typer(
        analytics_app,
        name="analytics",
    ...
    app.add_typer(notes_app, name="notes", help=f"Create and manage training notes.\n\nDocs: {CLI_DOCS['notes']}")
    app.add_typer(
        weather_app, name="weather", help=f"Fetch and manage activity weather data.\n\nDocs: {CLI_DOCS['weather']}"
    )
    app.add_typer(race_app, name="race", help=f"Race course management and simulation.\n\nDocs: {CLI_DOCS['race']}")
    app.add_typer(
        dashboard_app, name="dashboard", help=f"Launch local training dashboards.\n\nDocs: {CLI_DOCS['dashboard']}"
    )
    app.add_typer(backup_app, name="backup", help=f"Backup and restore FitOps data.\n\nDocs: {CLI_DOCS['backup']}")
