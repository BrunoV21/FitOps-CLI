...
def login(
    ...
    if not settings.client_id:
        ...
        typer.echo("")
        typer.echo("Step 1 — Create your Strava API application:")
        typer.echo("  Go to https://www.strava.com/settings/api and create an app with these settings:")
        ...
        typer.echo("    Authorization Callback Domain: mclovinittt-kinetic-run-api.hf.space")
        typer.echo("")
        typer.echo("Step 2 — Enter your credentials:")
        typer.echo("  Copy the Client ID and Client Secret from https://www.strava.com/settings/api")
        typer.echo("")
        if not client_id:
