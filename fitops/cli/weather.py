...
def _parse_latlng(latlng_json: str | None) -> tuple[float, float] | None:
    if not latlng_json:
        return None
    try:
...
def fetch_weather(
    ...
    if all_activities:
        ...
        if json_output:
            typer.echo(json.dumps(bulk_out, indent=2))
        else:
            print_weather_fetch_all(bulk_out)
        return

    if activity_id is None:
        typer.echo("Provide an activity ID or use --all.", err=True)
        raise typer.Exit(1)
    ...
    out = {"_meta": make_meta(), "weather": enriched}
    if json_output:
        typer.echo(json.dumps(out, indent=2))
    else:
        print_weather_activity(out)
...
def show_weather(
    ...
    async def _load() -> dict | None:
        ...
        return {
            **d,
            ...
            "course_bearing_deg": round(course_bearing, 1)
    ...
    if result is None:
        typer.echo(
            f"No weather data for activity {activity_id}.\n"
            f"  Run: fitops weather fetch {activity_id}",
            err=True,
        )
        raise typer.Exit(1)

    out = {"_meta": make_meta(), "weather": result}
    if json_output:
        typer.echo(json.dumps(out, indent=2))
    else:
        print_weather_activity(out)
...
def forecast_weather(
    ...
    hour: int = typer.Option(
        9, "--hour", help="Race start hour in local time (0-23). Default: 9."
    ),
    course_bearing: float | None = typer.Option(
        None,
        "--course-bearing",
    ...
    if temp_c is not None and humidity is not None:
        ...
        wap_factor = round(
            compute_wap_factor(temp_c, humidity, wind_speed, wind_dir, course_bearing),
            4,
        )
    ...
    if json_output:
        typer.echo(
            json.dumps({"_meta": make_meta(), "forecast": forecast_dict}, indent=2)
        )
    else:
...
def set_weather(
    ...
    if not weather:
        typer.echo(
            json.dumps({"error": "Provide at least one weather field."}, indent=2)
        )
        raise typer.Exit(1)
