...
class WorkoutSegment(Base):
    ...
    segment_name: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    step_type: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
    )  # warmup | interval | recovery | cooldown

    # Stream boundaries (indices into activity stream arrays)
    start_index: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    end_index: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)

    # Target
    target_focus_type: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
    )  # hr_zone | pace_zone | power_zone | rpe
    target_zone: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)

    # Pace / speed actuals
    avg_pace_per_km: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    pace_consistency_score: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True
    )  # 0.0–1.0

    # HR actuals
    avg_heartrate: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    hr_zone_distribution: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
    ...
    # Compliance
    target_achieved: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True
    )  # 1/0 bool stored as int (SQLite)
    deviation_pct: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True
    )  # positive = above target
    time_in_target_pct: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    time_above_pct: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    time_below_pct: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    compliance_score: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True
    )  # 0.0–1.0

    # Data quality
    has_heartrate: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    has_power: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    has_gps: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "compliance": {
                "target_achieved": bool(self.target_achieved) if self.target_achieved is not None else None,
